...
describe('Сheck order details', () => {
	it('Should return the initial state', () => {
		expect(orderDetailsSplice.reducer(undefined, { type: '' })).toEqual(
			initialState
		);
	});
	...
	it('Open modal details order', () => {
		expect(
			orderDetailsSplice.reducer(initialState, addOrderDetails(order))
		).toEqual({
			...initialState,
	...
	it('Close modal details order', () => {
		expect(
			orderDetailsSplice.reducer(
				{ ...initialState, orderDetail: orderDetail },
				deleteOrderDetails()
			)
	...
	it('Fulfilled details order', () => {
		expect(
			orderDetailsSplice.reducer(
				initialState,
				getOrder.fulfilled(order, '', {} as number)
			)
		).toEqual({ ...initialState, orderDetail: orderDetail });
	});
	...
	it('Rejected details order', () => {
		...
		expect(
			orderDetailsSplice.reducer(
				initialState,
				getOrder.rejected(new Error(message), '', {} as number)
			)
	...
	it('Rejected details order (message undefined)', () => {
		expect(
			orderDetailsSplice.reducer(
				initialState,
				getOrder.rejected(new Error(undefined), '', {} as number)
			)
