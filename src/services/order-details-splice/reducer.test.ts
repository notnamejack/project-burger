...
describe('Сheck create order', () => {
	...
	it('Open modal create order', () => {
		expect(orderSplice.reducer(initialState, addOrder({name: order.name, order: {number: order.number}}))).toEqual({
			...initialState,
	...
	it('Close modal details ingredient', () => {
		expect(orderSplice.reducer({ ...initialState, orderDetail: {name: order.name, order: {number: order.number}}}, deleteOrder())).toEqual(initialState);
	});
});