...
import {
	...
	getTotal,
	getTotalToday,
} from '../../services/tape-orders/slice';
...
export function Feed() {
	...
	const orderDone = useAppSelector(
		(state) =>
			state.tapeOrders.orders?.orders
				.filter(
					(i) => i.status.toLocaleUpperCase() === 'done'.toLocaleUpperCase()
				)
				.splice(0, 10) || []
	);
	const orderCreated = useAppSelector(
		(state) =>
			state.tapeOrders.orders?.orders
				.filter(
					(i) => i.status.toLocaleUpperCase() === 'created'.toLocaleUpperCase()
				)
				.splice(0, 10) || []
	);
				...
				<div className={clsx(s.panel)}>
					...
					<div className={clsx(s.history)}>
						<div>
							...
							<p className={`${clsx(s.result)} text text_type_digits-large`}>
								{total || ''}
							</p>
						</div>
						<div>
